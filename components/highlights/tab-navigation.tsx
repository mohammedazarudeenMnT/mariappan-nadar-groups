...
export function TabNavigation() {
  ...
  return (
    <div className="sticky top-20 sm:top-24  z-40 bg-white/95 backdrop-blur-md border-b border-base-200 shadow-sm dark:bg-base-950/95 dark:border-base-800">
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`relative px-4 sm:px-6 md:px-8 py-3 sm:py-4 text-xs sm:text-sm md:text-base font-medium whitespace-nowrap transition-all duration-300 flex-shrink-0 cursor-pointer ${
                activeTab === tab.id
                  ? "text-primary-600 scale-105 dark:text-primary-400"
                  : "text-base-600 hover:text-primary-500 hover:scale-102 dark:text-base-400 dark:hover:text-primary-400"
              }`}
            >
              {tab.label}
              {activeTab === tab.id && (
